...
const CreateItem = () => {
  ...
  useEffect(() => {
    if (!user) {
      ...
      // Format date to yyyy-MM-ddTHH:mm:ss for datetime-local
      ...
      const defaultAuctionEndTime = `${year}-${month}-${day}T${hours}:${minutes}:${seconds}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Format to ISO_LOCAL_DATE_TIME without timezone
      const formattedAuctionEndTime = auctionEndTime; // Already in yyyy-MM-ddTHH:mm:ss
      formData.append('auctionEndTime', formattedAuctionEndTime);
      formData.append('userId', user.id);
      if (image) {
        formData.append('image', image);
      }

      const response = await authFetch('http://localhost:8094/api/items/create', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md" encType="multipart/form-data">
        ...
        <div className="mb-4">
          <label className="block text-gray-700" htmlFor="startingBid">Starting Bid ($)</label>
          <input
            type="number"
            name="startingBid"
            id="startingBid"
        ...
        <div className="mb-4">
          <label className="block text-gray-700" htmlFor="auctionType">Auction Type</label>
          <select
            name="auctionType"
            id="auctionType"
        ...
        <div className="mb-6">
          <label className="block text-gray-700" htmlFor="auctionEndTime">
            Auction End Time {timeDifference && <span className="text-gray-600 text-sm">({timeDifference})</span>}
          </label>
          <input
            type="datetime-local"
            name="auctionEndTime"
            ...
            min={
              (() => {
                const now = new Date();
                now.setHours(now.getHours() + 1); // Allow bids as short as 1 hour
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="w-full px-3 py-2 border rounded mt-1"
          />
          {image && <p className="text-sm text-gray-600 mt-2">{image.name} selected</p>}
        </div>
        <button
          type="submit"
