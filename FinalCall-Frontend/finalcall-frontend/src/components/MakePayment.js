...
const MakePayment = () => {
    ...
    const fetchAuctionDetails = async () => {
      try {
        const response = await authFetch(
          `http://localhost:8084/api/auctions/${auctionId}`,
          { method: 'GET' },
          logout
        );
  ...
  const handlePayment = async (e) => {
    ...
    try {
      const response = await authFetch(
        `http://localhost:8083/api/payments/process-auction-payment/${auctionId}`,
        {
          method: 'POST',
          headers: {
      ...
      <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="mb-6">
          ...
          <p className="text-gray-700">
            Amount to Pay: ${auctionDetails.currentBidPrice.toFixed(2)} USD
          </p>
        </div>
        ...
        <form onSubmit={handlePayment}>
          ...
          {/* Conditional Fields for Credit or Debit Card */}
